...
def create_control_node(vocab, theta=0.9):
    d = vocab.dimensions
    let_me_out = []
    ...
    def flag_switcher(t, x):
        ...
        flag = x[-1]
        header = spa.SemanticPointer(x[d:d*2])
        elseb = vocab['F_ELSE']
        thenb = vocab['F_THEN']
        ...
        if controlcode @ empty > theta:
            ...
            cos_sim2 = vcos(controlcode, branch2)
            cos_sime = 0
            cos_simt = 0
            
            if header @ header > theta: 
                cos_sime = vcos(header, elseb)
                cos_simt = vcos(header, thenb)

            if cos_sim1 > cos_sim2 and cos_sim1 > theta:
                flag = 0
                if x[-1] > 0.91:
                    #print ('Hey Penti')
                    flag = 1
                if cos_sime > cos_simt and cos_sime > theta:
                    #print('we did it')
                    flag = 1
    ...
    def control_da_flow(t, x):
        nonlocal let_me_out
        header = x[d:d*2]
        peep = vocab['S_PEEK'].v
        flag = flag_switcher(t,x)
        if flag == 0:
            temp = np.concatenate((header, peep))
            let_me_out = np.append(temp, flag)
        elif flag == 1:
            temp = np.concatenate((np.zeros(len(header)), peep))
            let_me_out = np.append(temp, flag)
        return let_me_out
    return control_da_flow
...
<<<<<<< HEAD
<<<<<<< HEAD
            #we may be in the if/else/then twilight state 
            control_check = nengo.Node(
                    create_control_node(voc),
                    size_in=2*voc.dimensions + 1, 
                    size_out=2*voc.dimensions + 1, 
            ...
            nengo.Connection(output_state.output, control_check[d:d*2])
            nengo.Connection(control_check[:d], mod_node[:d])
            nengo.Connection(control_check[-1], control_check[-1])
...
=======
            #def cleanup_node(t, x, vocab=self.voc):
            #    return cleanup(x, vocab=voc).v
>>>>>>> 3ac58ccf38b421d6e2c8a6bc511458b1a394470c
...
<<<<<<< HEAD
"""
STACK: The below functions and class definitions define the stacks that are used for the FORTH interpreter.
The stacks used include: data_stack (for holding the data to be processed), call_stack (), 
ctrl_flow_stack (), and a return_stack (). 

SimpleStack defines the base stack class which stacks are made from.
"""
...
class DropCircuit(WordCircuit):
    """
    Drops top element of the stack
    """
<<<<<<< HEAD
=======
...
<<<<<<< HEAD
    ...
    nengo.Connection(control_unit.trigger, ctrl_flow_stack.sigin)

    nengo.Connection(ctrl_flow_stack.output, control_unit.flag_control[:d])
